...
namespace Mohid
{
   public class MyWaterTamegaEVTPComparison : BCIMohidSim
   {
      ...
      protected bool CopyMeteoFiles(MohidRunEngineData mre)
      {
         ...
         orig.FullPath = @"..\general.data\boundary.conditions\" + mre.sim.Start.ToString("yyyyMMddHH") + "_" + mre.sim.End.ToString("yyyyMMddHH") + @"\meteo.hdf5";
      ...
      //   tool.Output = @"..\..\7\local.data\boundary.conditions\mm5.glued.hdf5";
      //   tool.Is3DFile = false;
      ...
      //   if (!FileTools.CopyFile(new FileName(@"..\7\templates\tools\convert.to.HDF5.action.template"), new FileName(@"..\tools\interpolation\converttoHDF5action.dat"), CopyOptions.OVERWRIGHT))
      //      return false;
