...
namespace Mohid
{
   public class OnlyRefEVTPfromCPTEC : BCIMohidSim
   {
      ...
      public override bool OnSimStart(object data)
      {
         ...
         FilePath destiny = new FilePath();
         FilePath source = new FilePath();
         source.Path = @"L:\Portugal\Douro\Tamega\MyWater\Simulations\MohidLand\Ref.Evtp\store\" + mre.sim.Start.AddSeconds(1).ToString("yyyyMMdd.HHmmss") + "-" + mre.sim.End.AddSeconds(1).ToString("yyyyMMdd.HHmmss");
         destiny.Path = @"L:\Portugal\Douro\Tamega\MyWater\Simulations\MohidLand\Ref.Evtp.3m.4\general data\Boundary Conditions\";

         if (!FileTools.CopyFile(source, destiny, "meteo_mm5.hdf5", CopyOptions.OVERWRIGHT))
            return false;
      ...
      public bool GenerateCumulative(MohidRunEngineData mre)
      {
         ...
         toGlue.FullName = "basinevtp.hdf5";
         Console.WriteLine("{0}", toGlue.FullPath);
         if (!GlueFiles(tool, false, workingFolder, destinationFolder, toGlue, mre))
            return false;
