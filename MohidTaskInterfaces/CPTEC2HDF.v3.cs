...
namespace CPTEC2HDF_Xh_v3
{
   ...
   public class CPTEC2HDFv3 : IMohidTask
   {
      ...
      bool IMohidTask.Run(ConfigNode cfg)
      {
         ...
         for (; ; )
         {
            ...
            {
               if (!RunConversionForAllFiles())
               {
                  result = false;
                  break;
               }
            }
            else if (!RunForFileList())
            {
               result = false;
               break;
            }
      ...
      protected bool LoadGlobalConfig()
      {
         try
         {
            ...
            convert_all_files = cfg["convert.all.files", true].AsBool();
      ...
      protected bool RunConversionForAllFiles()
      {         
         bool errors = false;
         int index = 0;

         store_path.Path = store_folder.Path;
