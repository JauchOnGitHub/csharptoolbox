   ...
   public class CPTEC2HDFv2 : IMohidTask
   {
      ...
      public bool CheckUserInput()
      {
         ...
         if (!Directory.Exists(fCPTECFilesPath))
            return false;
         if (!fCPTECFilesPath.EndsWith(System.IO.Path.DirectorySeparatorChar))
            fCPTECFilesPath += System.IO.Path.DirectorySeparatorChar;

         if (!Directory.Exists(fOutputPath))
            return false;
         if (!fOutputPath.EndsWith(System.IO.Path.DirectorySeparatorChar))
            fOutputPath += System.IO.Path.DirectorySeparatorChar;
         ...
         if (!Directory.Exists(fGlueExePath))
            return false;
         if (!fGlueExePath.EndsWith(System.IO.Path.DirectorySeparatorChar))
            fGlueExePath += System.IO.Path.DirectorySeparatorChar;

         if (!Directory.Exists(fGlueWorkingFolder))
            return false;
         if (!fGlueWorkingFolder.EndsWith(System.IO.Path.DirectorySeparatorChar))
            fGlueWorkingFolder += System.IO.Path.DirectorySeparatorChar;
