...
namespace Mohid
{
   public class ScriptV2: BCIMohidSim
   {
         ...
         try
         {
            ...
            if (join_ts_list != null && join_ts_list.Count > 0)
            {
               ...
               {                  
                  ...
                  if (JoinTimeseriesByFolder(output, filter, to_search, search_sub_folders, overwrite, tu))                  
      ...
      protected bool JoinTimeseriesByFolder(FileName output, string filter, List<FilePath> folders_to_search, bool search_sub_folders = true, bool overwrite = true, TimeUnits tu = TimeUnits.SECONDS)
      {
         try
         {
         ...
         {
            exception = new Exception("JoinTimeseries failed.", ex);
            return false;
         }
      }
